...
TARGET_MODEL_PATH = model_dir + '/Mistral-7B-Instruct-v0.3-Q4_K_M.gguf'
# !!! Используем существующую модель DeepSeek как черновик
DRAFT_MODEL_PATH = model_dir + '/tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf'


# 1. Инициализация Черновой модели (Draft Model)
# На CPU-машине обязательно устанавливаем n_gpu_layers=0!
draft_llm = Llama(
    model_path=DRAFT_MODEL_PATH,
    n_ctx=8192,
    n_batch=1024,
    n_gpu_layers=0, # <-- КРИТИЧЕСКИ ВАЖНО для CPU-сценария
    verbose=False,
)

# 2. Инициализация Основной модели (Target Model) с Speculative Decoding
llm = Llama(
    ...
    n_threads=10,
    verbose=False,
    draft_model=draft_llm, # <-- Включаем Speculative Decoding с DeepSeek
)
...
def promtConstructor(query: str) -> str:
    print(f"model = {TARGET_MODEL_PATH.split('/')[-1]}")
    if 'Mistral' in TARGET_MODEL_PATH:
...
if __name__ == "__main__":
    ...
    tests = ['джили монджаро 2023 масло моторное', 'opel astra h 1.6 масло моторное', 'bmw x5 2012 масло моторное',
             'Масло трансмиссионное CVT Chery Tiggo ', 'масло моторное Nisan Maxima 2013',
             'Nissan X-Trail 2012 масло моторное и масло вариантора',
             'масло моторное ford Transit 2013', "Audi Q7 2010 масло моторное и трансмисионное",
             'жидкость для вариатора Toyota Corolla 2012',
